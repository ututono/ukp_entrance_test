...
from src.utils_.global_variables import OUTPUT_DIR_NAME, VAL_CM_NAME, TRAIN_CM_NAME
...
class UtilsTestCase(unittest.TestCase):
    ...
    def test_visualize_cm(self):
        KEY_NAME = 'macro_f1'
        ckpt_dir = utils.get_ckpt_dir(date_time='2023_11_28-15_10_00')
        val_cm = utils.json2dict(os.path.join(ckpt_dir, VAL_CM_NAME))
        utils.visualize_cm(KEY_NAME, val_cm, f"{KEY_NAME} on Dev set",os.path.join(ckpt_dir, 'val_marco_f1.png'))

        train_cm = utils.json2dict(os.path.join(ckpt_dir, TRAIN_CM_NAME))
        utils.visualize_cm(KEY_NAME, train_cm, f"{KEY_NAME} on Train set",os.path.join(ckpt_dir, 'train_marco_f1.png'))
